...
import { getActiveVouchers, purchaseVoucher, getUserVouchers, markVoucherAsUsed, getVoucherById, type UserVoucher, type Voucher } from '@/lib/firebase/database/vouchers';
...
export default function UserVouchersPage() {
  ...
  const text = {
    ...
    voucherPurchaseSuccess: isHebrew ? 'השובר נרכש בהצלחה!' : 'Voucher purchased successfully!',
    ...
    voucherMarkedAsUsed: isHebrew ? 'השובר סומן כמשומש והועבר להיסטוריה' : 'Voucher marked as used and moved to history',
  ...
  const [coupons, setVouchers] = useState<Coupon[]>([]);
  ...
  const [selectedVoucherImage, setSelectedVoucherImage] = useState<{ imageUrl: string; name: string; description?: string; voucher?: Voucher; userVoucher?: UserVoucher } | null>(null);
  ...
  const fetchData = async () => {
    try {
      ...
      const couponsResult = await getActiveVouchers();
      ...
      if (couponsResult.success && couponsResult.coupons) {
        ...
        const couponsWithDates = couponsResult.coupons.map((coupon: any) => {
          ...
          if (voucher.businessIds) {
            if (Array.isArray(voucher.businessIds)) {
              normalizedBusinessIds = voucher.businessIds.filter((id: any) => id != null && id !== '');
            } else if (typeof voucher.businessIds === 'string') {
              normalizedBusinessIds = [voucher.businessIds];
            }
          } else if (voucher.businessId) {
            // Legacy format: convert single businessId to array
            normalizedBusinessIds = [voucher.businessId];
          }
          
          // Debug: Log specific coupon
          if (voucher.id === 'IvmgfeBPfGRXLIQ5ce0Q') {
            console.log('🔍🔍🔍 CLIENT - Parsing coupon IvmgfeBPfGRXLIQ5ce0Q:', {
              rawBusinessIds: voucher.businessIds,
              rawBusinessId: voucher.businessId,
              normalizedBusinessIds,
              normalizedLength: normalizedBusinessIds?.length,
              isArray: Array.isArray(voucher.businessIds),
              type: typeof voucher.businessIds,
        ...
        couponsWithDates.forEach(coupon => {
          console.log(`Coupon: ${voucher.name}`, {
            id: voucher.id,
            ...
            price: voucher.price,
            hasBusinessId: !!voucher.businessId,
            hasBusinessIds: !!voucher.businessIds,
            businessId: voucher.businessId,
            businessIds: voucher.businessIds
          });
        });
        ...
        const validCoupons = couponsWithDates.filter(coupon => {
          ...
          const isValid = voucher.isActive && validFrom <= now && validTo >= now;
          
          console.log(`Checking coupon "${voucher.name}":`, {
            isActive: voucher.isActive,
      ...
      if (user) {
        ...
        const historyResult = await getUserVouchers(user.uid);
        if (historyResult.success && historyResult.coupons) {
          ...
          const allCoupons = historyResult.coupons.map(uc => ({
            ...
            coupon: {
              ...uc.coupon,
              validFrom: new Date(uc.voucher.validFrom as any),
              validTo: new Date(uc.voucher.validTo as any),
            },
          ...
          const validCoupons = allCoupons.filter(uc => {
            ...
            const validTo = new Date(uc.voucher.validTo);
            return validTo >= now;
          });
          
          // Sort: free vouchers first, then by purchasedAt descending (newest first)
          validCoupons.sort((a, b) => {
            const aIsFree = a.voucher.price === 0;
            const bIsFree = b.voucher.price === 0;
  ...
  const handlePurchaseCoupon = async (coupon: Coupon) => {
    ...
    const pointsNeeded = freeCouponPrice ? 0 : voucher.points;

    if (!freeCouponPrice && userPoints < voucher.points) {
    ...
    try {
      ...
      if (!purchaseResult.success) {
        ...
        if (!freeCouponPrice) {
          await addPointsToCategory(user, 'share', voucher.points);
        }
        toast.error(purchaseResult.error || text.failedToPurchase);
        return;
      }

      toast.success(isHebrew ? `השובר "${voucher.name}" נרכש בהצלחה!` : `Voucher "${voucher.name}" purchased successfully!`);
  ...
  const handleUseCoupon = async (userVoucher: UserCoupon) => {
    ...
    <div className="min-h-screen bg-background">
      ...
      <Tabs value={activeTab} onValueChange={(value) => {
        ...
        <TabsContent value="shop" className="space-y-6">
          <h2 className="text-3xl lg:text-4xl font-bold mb-6 lg:mb-8 text-gray-900">{text.availableVouchers}</h2>
        {coupons.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6 lg:gap-8 pb-24">
            {coupons.map((coupon) => (
              ...
              >
                {voucher.imageUrl && (
                  <div 
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      setSelectedVoucherImage({ imageUrl: voucher.imageUrl, name: voucher.name, description: voucher.description, coupon });
                    }}
                  >
                    <img 
                ...
                <CardHeader className={voucher.imageUrl ? "pb-3" : ""}>
                  {!voucher.imageUrl && (
                  ...
                  <div className="flex items-center justify-between gap-2 text-sm">
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                      <span className="font-semibold text-gray-900 truncate">{voucher.name}</span>
                    </div>
                    <div className="flex items-center gap-3 flex-shrink-0">
                      {voucher.price !== 0 && (
                        <Badge variant="outline" className={`flex items-center gap-1 px-2 py-0.5 text-xs ${freeCouponPrice ? 'border-green-200 text-green-700' : 'border-amber-200 text-amber-700'}`}>
                          ...
                          <span>{freeCouponPrice ? '0' : voucher.points}</span>
                        </Badge>
                      )}
                    </div>
                  </div>
                  {voucher.description && (
                    <p className="text-sm text-gray-600 mt-2 line-clamp-2">{voucher.description}</p>
                  )}
                </CardHeader>
                <CardContent className="space-y-4" onClick={(e) => e.stopPropagation()}>
                  <div className="space-y-3">
                    <div className={`flex items-center justify-between p-3 rounded-lg ${voucher.price === 0 ? 'bg-green-50' : 'bg-amber-50'}`}>
                      ...
                      <Badge variant="outline" className={`flex items-center gap-1.5 bg-white px-3 py-1 ${freeCouponPrice ? 'border-green-200 text-green-700' : 'border-amber-200 text-amber-700'}`}>
                        ...
                        <span className="font-semibold">{freeCouponPrice ? '0' : voucher.points}</span>
                      </Badge>
                      )}
                    </div>
                    {freeCouponPrice && (
                ...
                <CardFooter className="pt-4 mt-auto flex flex-col gap-2" onClick={(e) => e.stopPropagation()}>
                  <Button 
                    ...
                    disabled={!freeCouponPrice && userPoints < voucher.points}
                  ...
                  >
                    ...
                    {freeCouponPrice 
                      ? text.getFree
                      : (userPoints < voucher.points ? text.insufficientPoints : text.purchase)
                    }
                  </Button>
                  <Button
                    variant="outline"
                    ...
                    onClick={async () => {
                      ...
                      if (!hasBusinessIds && !hasBusinessId && couponToUse?.id) {
                        ...
                        const result = await getVoucherById(couponToUse.id);
        ...
        <TabsContent value="myCoupons" className="space-y-6">
          <h2 className="text-3xl lg:text-4xl font-bold mb-6 lg:mb-8 text-gray-900">{text.myVouchers}</h2>
          {couponHistory.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6 lg:gap-8">
              {couponHistory.map((userVoucher) => {
                const coupon = userVoucher.voucher;
                // Ensure status is properly set (default to 'active' if missing or invalid)
                const status = userVoucher.status || 'active';
                ...
                const couponCode = voucher.description;
                ...
                return (
                  ...
                  >
                    ...
                    {voucher.imageUrl && (
                      <div 
                        ...
                        onClick={(e) => {
                          e.preventDefault();
                          e.stopPropagation();
                          setSelectedVoucherImage({ imageUrl: voucher.imageUrl, name: voucher.name, description: voucher.description, userVoucher });
                        }}
                      >
                        <img 
                          src={voucher.imageUrl} 
                    ...
                    <CardHeader className={voucher.imageUrl ? "pb-3" : "pt-6"}>
                      {!voucher.imageUrl && (
                        <div className="flex items-start justify-between gap-3 mb-2">
                      ...
                      <div className="space-y-4 flex-1">
                        ...
                        <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                          ...
                          <span className={`text-lg font-bold ${
                            isActive && !isExpired 
                              ? (voucher.price === 0 ? 'text-green-600' : 'text-primary')
                              : 'text-gray-600 line-through'
                          }`}>
                            {voucher.price === 0 ? text.free : formatPrice(voucher.price)}
                          </span>
                        </div>
                      </div>
                    </CardContent>
                    <CardFooter className="pt-4 flex flex-col gap-2 mt-auto" onClick={(e) => e.stopPropagation()}>
                      <Button
                        variant="outline"
                        size="lg"
                        onClick={() => navigate(`/${locale}/vouchers/${userVoucher.id}`)}
                        className="w-full h-12 flex items-center justify-center gap-2"
                      >
      ...
      {isDesktop ? (
        <Dialog open={!!selectedVoucherImage} onOpenChange={(open) => !open && setSelectedVoucherImage(null)}>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            {selectedVoucherImage && (
              <>
                <DialogHeader>
                  <DialogTitle className="text-2xl font-bold">{selectedVoucherImage.name}</DialogTitle>
                </DialogHeader>
                <div className="space-y-4">
                  {/* Image */}
                  {selectedVoucherImage.imageUrl && (
                    <div className="rounded-lg overflow-hidden bg-gray-100">
                      <img 
                        src={selectedVoucherImage.imageUrl} 
                        alt={selectedVoucherImage.name}
                  ...
                  {/* Description */}
                  {selectedVoucherImage.description && (
                    <div>
                      <p className="text-base text-gray-700">{selectedVoucherImage.description}</p>
                    </div>
                  )}
                  ...
                  <div className="flex items-center gap-2 text-gray-600">
                    ...
                    <span className="text-sm">
                      {text.validUntil}: {
                        selectedVoucherImage.coupon 
                          ? formatDate(selectedVoucherImage.voucher.validTo)
                          : selectedVoucherImage.userVoucher?.coupon
                          ? formatDate(selectedVoucherImage.userVoucher.voucher.validTo)
                          : ''
                      }
                    </span>
                  </div>
                  ...
                  {selectedVoucherImage.coupon && (
                    <div className="p-4 bg-gray-50 rounded-lg">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-medium text-gray-600">{text.pointsRequired}</span>
                        {selectedVoucherImage.voucher.price === 0 ? (
                          ...
                          <Badge variant="outline" className={`flex items-center gap-1.5 ${freeCouponPrice ? 'border-green-200 text-green-700' : 'border-amber-200 text-amber-700'}`}>
                            <Coins className="h-4 w-4" />
                            <span>{freeCouponPrice ? '0' : selectedVoucherImage.voucher.points}</span>
                          </Badge>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 pt-4">
                    {selectedVoucherImage.coupon && (
                      <Button
                        onClick={() => {
                          if (selectedVoucherImage.coupon) {
                            handlePurchaseCoupon(selectedVoucherImage.coupon);
                            setSelectedVoucherImage(null);
                          }
                        }}
                        disabled={!freeCouponPrice && selectedVoucherImage.coupon && userPoints < selectedVoucherImage.voucher.points}
                      ...
                      >
                        ...
                        <span className="font-semibold">
                          {freeCouponPrice 
                            ? text.getFree 
                            : (selectedVoucherImage.coupon && userPoints < selectedVoucherImage.voucher.points 
                    ...
                    {selectedVoucherImage.userVoucher && (
                      <Button
                        variant="outline"
                        onClick={() => {
                          navigate(`/${locale}/vouchers/${selectedVoucherImage.userVoucher!.id}`);
                          setSelectedVoucherImage(null);
                        }}
        ...
        <Drawer open={!!selectedVoucherImage} onOpenChange={(open) => !open && setSelectedVoucherImage(null)}>
          <DrawerContent className="max-h-[85vh]">
            {selectedVoucherImage && (
              <>
                <DrawerHeader className="pb-4">
                  <div className="flex items-center justify-between">
                    <DrawerTitle className="text-2xl font-bold">{selectedVoucherImage.name}</DrawerTitle>
                    <Button
                      variant="ghost"
                      size="icon"
                      onClick={() => setSelectedVoucherImage(null)}
                      className="h-8 w-8"
                    >
                      <X className="h-5 w-5" />
                    </Button>
